...
```bash
docker exec -it rabbitmq rabbitmqctl add_user <username-here> <passsword-here>
docker exec -it rabbitmq rabbitmqctl set_user_tags admin administrator
docker exec -it rabbitmq rabbitmqctl set_permissions -p / <username-here> ".*" ".*" ".*"
```
...
```bash
flask --app app.flask_app:create_app run --reload
```
...
```bash
flask --app app.flask_app:create_app run --reload -p 5001
```
...
```bash
curl -X POST http://localhost:5001/parse_exploits
```
...
```bash
curl -X GET http://localhost:5001/check_task/1
```
